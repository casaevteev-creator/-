...
/* Раньше здесь ждали «Файл можно загружать». Теперь приложение читает сам код,
   дописанный в правила обмена, и видит настоящий блокер: все приходные ордера
   станут розничной выручкой. Пока это не поправят в УМЦ, файл грузить нельзя,
   и отчёт обязан так и говорить. */
if (!текст3.includes("Правила обмена сделают 8 приходных ордеров"))
  беда.push("август: не назван блокер — приходные ордера станут выручкой");
if (!текст3.includes("Загружать рано"))
  беда.push("август: вердикт не изменился, хотя блокер найден");
...
await стр5.selectOption("#c-podr", "ордера");
...
/* названия берутся из самих правил обмена, а не из комментария выгрузки */
for (const п of ["Основное подразделение", "Самокатная 1 стр.1"])
...
const кассыXML = await стр5.evaluate(() => {
  ...
  for (const o of x.getElementsByTagName("Объект")){
    ...
    const p = [...o.children]
      .find(s => s.getAttribute("Имя") === "ПодразделениеОрганизации");
    if (!p) continue;
    всего++;
    if (p.getAttribute("ИмяПКО") !== "ПодразделенияОрганизаций") безВладельца += 1000;
...
/* разбор самих правил обмена: там дописан код, который переписывает документы
   уже после загрузки, и человек должен узнать об этом до кнопки «скачать» */
for (const кусок of ["Что сделают с документами сами правила обмена",
                     "Форма строение 12 → Основное подразделение",
                     "Форма строение 1 → Самокатная 1 стр.1",
                     "вид операции на «РозничнаяВыручка»",
                     "Эквайринг БАНК ВТБ"])
  if (!текст5.includes(кусок))
    беда.push("правила обмена: в отчёте нет «" + кусок + "»");
if (!текст5.includes("через 57.01 не пройдут"))
  беда.push("правила обмена: не сказано, что «Прочий приход» перебьётся");
if (кассыXML.безВладельца)
  беда.push("кассы: ссылок на подразделение без владельца — " + кассыXML.безВладельца);
for (const тип of ["ПриходныйКассовыйОрдер", "РасходныйКассовыйОрдер"])
  if (!кассыXML.поТипам[тип])
    беда.push("кассы: подразделение не проставлено в «" + тип + "»");
/* отчёты о розничных продажах правила делят сами — свой реквизит им дописывать
   нельзя, на нём загрузка в бухгалтерию и падала */
if (кассыXML.поТипам["ОтчетОРозничныхПродажах"])
  беда.push("кассы: подразделение дописано в отчёт о розничных продажах — так нельзя");
...
console.log("всё сошлось: сентябрь — карточки, " + ПРАВОК + " правок, наличные и терминалы " +
            ...
            "деление по кассам через подразделения; разбор дописанного кода в правилах обмена; " +
            "месяц, обрезанный до одного дня");
