...
# 4в. разбор в автономной версии не считается — раздел заменяется объяснением
html = html.replace("${advSection()}", """<section class="tight advisor"><div class="w">
    <div class="kick">\u00a7\u00a7 · Сборка</div><h2>Комментарий к месяцу</h2>
    <div class="sb">В автономной версии сводка не собирается — но все цифры для неё можно унести с собой</div>
    <div class="dnote"><b>Кнопка «Сводка месяца текстом»</b> на рабочем экране копирует и сохраняет всё,
    на что опирается этот отчёт: выручку по группам и каналам, ряд по дням, статьи расходов, историю по месяцам
    и незакрытые сверки. Комментарий к месяцу пишется руками в поле на рабочем экране и попадает в отчёт
    сразу после резюме. Если открыть программу по ссылке, сводка собирается сама.</div>
  </div></section>""")
